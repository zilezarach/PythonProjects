...
def delay_print(s):
    for c in s:
            ...
            sys.stdout.flush()
            time.sleep(0.1)
delay_print('\n\n\n\n\n\n\nWithin the depths of darkness, hidden truths silently await -Zarachii')
console.clear()
console.print('[red]' + str(fy) + '[/]')
...
def GetBalance(address):
      reciev= requests.get(f"https://btc4.trezor.io/api/v2/address/{address}").json()
      return dict(reciev)['balance']

def GetTxts(address):
      try:
            reciev = requests.get(f"https://btc4.trezor.io/api/v2/address/{address}").json()
            return dict(reciev)["txs"]
      except:
            return '0'
...
while True: 
    ...
    for i in range(0, 1):
          ...
          Bib32_Child_Key_Object = Bip32_Root_Key_Object.ChildKey(44 + bip32utils.BIP32_HARDEN).ChildKey(0 + bip32utils.BIP32_HARDEN).ChildKey(0 + bip32utils.BIP32_HARDEN).ChildKey(0).ChildKey(i)
          First_encode = base58.b58decode(Bib32_Child_Key_Object.WalletImportFormat())
          Private_Key_Byte = binascii.hexlify(First_encode)
          Private_Key_Hex = Private_Key_Byte[2:-10]
          jam = int(Private_Key_Hex.decode(),16)
          bip32_root = Bip32_Root_Key_Object.ExtendedKey()
          public_key = binascii.hexlify(Bip32_Root_Key_Object.PublicKey()).decode()
          ...
          bit_UCom = Key(wifUncompressed)
          compressedAddr = bit_COm.address
          UncompressedAddr = bit_UCom.address
          txs_Compressed = GetTxts(compressedAddr)
          txs_Uncompressed = GetTxts(UncompressedAddr)
